...
#define ESP_GATT_UUID_SPP_COMMAND_NOTIFY    0xFFE0
        ...
        if (xQueueReceive(spp_uart_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
            ...
            case UART_DATA:
                if ((event.size)&&(is_connected)) {
                    ...
                    temp = (uint8_t *)malloc(/*sizeof(uint8_t)*event.size*/20);
                    if(temp == NULL){
...
*/
	...
	xQueueBtClient = xQueueCreate(10, sizeof(uint32_t));
    xTaskCreate(Debug, "Debug", 2048, NULL, 8, NULL);
}
#endif
...
void spp_cmd_task(void * arg)
{
    ...
    for(;;)
    {
        if(xQueueReceive(xQueueBtClient, &cmd_id, portMAX_DELAY))
        {
            ...
            if(p_data->write.is_prep == false){
               /* ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_WRITE_EVT : handle = %d\n", res);*/
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                	...
                	else
                	{
						...
						else
						{
							...
							else
							{
								...
								{
									if(strstr((const char*)spp_cmd_buff,"$ERASE KEYLESS:") != NULL)
									{
										/* Receive data over BT and printout over UART*/
										stBleMsg.ucSrc = SRC_BLE;
										stBleMsg.ucDest = SRC_APP;
										stBleMsg.ucEvent = EVENT_APP_BLE_ERASING_KEYLESS;
										stBleMsg.pcMessageData = (char*)spp_cmd_buff;
                    ...
                    /*xQueueSend(xQueueBtClient,&spp_cmd_buff,10/portTICK_PERIOD_MS);*/
...
void BleInit(void)
{
    ...
    xTaskCreate(vTaskBle, "vTaskBle", 2048, NULL, 8, NULL);
}
...
void vTaskBle( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		if(xQueueReceive(xQueueBle, &( stBleMsg ), portMAX_DELAY))
		{
			...
			sprintf((char*)u8DataBuffer,"%s",stBleMsg.pcMessageData);
			szSize = strlen((const char*)u8DataBuffer);
			esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],szSize, u8DataBuffer, false);
		}
